    ...
    methods: {
      getData(){
        ...
        if((self.params.startMonth!='')&&(self.params.endMonth!='')){
          ...
          if(parseInt(self.params.startMonth) > parseInt(self.params.endMonth)){
            alert("开始时间不能晚于结束时间");
            return ;
          }else{
        ...
        columnsTemp.push({title: '次数', key: 'loginCount', align: 'center', children: monthList.reverse()});
              ...
              data.forEach(rec => {
                ...
                dataListTemp.forEach(r => {
                  if(r[self.statConfig[self.statType].name] === name) {
                    r[rec.statMonth] = rec.loginCount;
                    flag = true;
                  }
                });
                if(!flag) {
                  d[self.statConfig[self.statType].name] = name;
                  d[rec.statMonth] = rec.loginCount;
                  dataListTemp.push(d);
                }
              });
